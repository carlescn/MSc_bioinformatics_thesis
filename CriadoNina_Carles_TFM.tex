...
\chapter{Materials i mètodes}
    ...
    \label{s:metodes_vae}
        ...
        Podem escollir que aquesta distribució sigui coneguda,
...
\todo{Inserir imatge $p(x) \rightarrow q(z|x) \rightarrow p(z) \rightarrow p(x|z)$}
        ...
        \subsubsection{Optimització dels paràmetres}
        \label{s:vae_parametres}

            El codificador pretén aprendre els paràmetres $\theta$ òptims
            de la distribució $p_\theta(\mathbf{z|x})$.
            Aquesta distribució bé condicionada per la distribució marginal $p_\theta(\mathbf{x})$
            en la equació:
            \begin{equation}
            ...
            Aquesta distribució marginal bé donada per:
            \begin{equation}
            ...
            En el seu lloc,
            podem inferir un model $q_\phi(\mathbf{z|x})$
        ...
        \subsubsection{El terme \acrshort{elbo}}
        \label{s:vae_elbo}

            Per optimitzar els paràmetres del \gls{vae}
            s'utilitza un terme anomenat \gls{elbo}:
            ...
            Aquest terme es relaciona amb la distribució marginal $p_\theta(\mathbf{x})$ per la igualtat:
            ...
            On el terme $D_{KL} ( q_\phi(\mathbf{z|x}) \| p_\theta(\mathbf{x|z} )$
            és la divergència de \gls{kl}
            entre $q_\phi(\mathbf{z|x})$ i $p_\theta(\mathbf{x|z})$.
            Aquest terme és sempre no-negatiu
            i és zero si i només si $q_\phi(\mathbf{z|x}) = p_\theta(\mathbf{z|x})$.

            Per tant, el terme \gls{elbo} és el limit inferior de $\log p_\theta(\mathbf{x})$:

            \begin{equation}
            \label{eq:elbo_lim}
                \mathcal{L}_{\theta,\phi}(\mathbf{x}) \le
                \log p_\theta(\mathbf{x})
            \end{equation}

            En conseqüència, maximitzant el terme \gls{elbo}
            amb respecte als paràmetres $\phi$ i $\theta$
            s'aconsegueix optimitzar dues característiques:

            \begin{itemize}
                \item Minimitzar la divergència de \gls{kl}
                entre $q_\phi(\mathbf{z|x})$ i $p_\theta(\mathbf{z|x})$,
                de manera s'aconsegueix millorar $q_\phi(\mathbf{z|x})$
                i el codificador obté una millor representació.

                \item Maximitzar la distribució marginal $p_\theta(\mathbf{x})$,
                de manera que el codificador aconsegueix
                una millor reconstrucció.
            \end{itemize}

        \subsubsection{Reparametrització}
        \label{s:vae_reparametritzacio}

\todo{
    Explicar el reparametrization trick
    en funció del descens de gradients
    i les transformacions deterministes
    vs el soroll aleatori.
    No intentar entrar l'explicació matemàtica
    ja que s'ha d'entrar en massa detall per què tingui sentit.
 }

            El terme \gls{elbo} es pot optimitzar
            amb respecte als paràmetres $\phi$ i $\theta$
            mitjançant el descens de gradients.
            Els gradients respecte als paràmetres $\phi$
            es poden obtenir mitjançant un estimador simple
            (de nou, no s'entra en detall en l'explicació).

            Per contra, els gradients respecte als paràmetres $\theta$
            són difícils de computar
            i es requereix realitzar una reparametrització,
            és a dir un canvi de variables.




    \section{Clustering loss?}
    \label{s:??}
